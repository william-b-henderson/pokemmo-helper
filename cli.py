...
def find_closest_word(search_string: str, data: dict):
  ...
  """
  closest_match = min(data.keys(), key=lambda word: levenshtein_distance(search_string, word))
...
def print_route_pokemon(route_dict: dict):
  for location in route_dict:
    ...
    for pokemon in route_dict[location]:
      ...
      horde = pokemon["horde"]
      time = pokemon["time"]
      print(f"Pokemon: {pokemon_name} ({rarity}) {'(Horde)' if horde else ''} {time if time else ''} {f'[{items}]' if items else ''}")
